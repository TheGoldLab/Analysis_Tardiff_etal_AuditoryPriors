...
bound_multiplier = 300;
if nargout > 2
    y      = (6 * data_(:,end)) - 3;
    inits_ = [(data_(:,1:end-1)\y), ....
        -bound_multiplier*ones(size(data_,2)-1,1) bound_multiplier*ones(size(data_,2)-1,1)];  % this is a 1st guess
else
...
if ischar(lumode)
    switch lumode
        ...
        case 'l'
            fun_   = @logist_valL;
            inits_ = cat(1, inits_, [0.01 0 max_lapse]);
        case 'u'
            fun_   = @logist_valU;
            inits_ = cat(1, inits_, [0.01 0 max_lapse]);
        case 'lug'
        ...
        case 'lu2'
            fun_   = @logist_valLU2;
            inits_ = cat(1, inits_, [0.01 0 max_lapse; 0.01 0 max_lapse]);
        otherwise
            fun_   = @logist_val;
    end
end
