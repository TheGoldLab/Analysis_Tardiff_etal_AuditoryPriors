function dat2Fit=make_precue_adapt_dmat(data)
    dummy = ones(height(data), 1);
    dat2Fit = repmat([dummy data.aSNR],1,3); %repeat design mat per condition
    dat2Fit = [dat2Fit data.zSNR data.choice01]; %add choice data
